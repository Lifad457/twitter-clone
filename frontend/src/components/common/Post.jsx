...
const Post = ({ post }) => {
	...
	const { mutate: likePostMutation, isPending: isLinking } = useMutation({
		mutationFn: async () => {
			try {
		...
		onSuccess: (updatedLikes) => {
			// not the best way to do this, becaause it will re-render the whole page
			// queryClient.invalidateQueries({ queryKey: ['posts'] });
	...
	const { mutate: commentOnPostMutation, isPending: isCommenting } =
		useMutation({
			mutationFn: async () => {
				try {
					...
					if (!response.ok)
						throw new Error(
							data.message || 'Failed to comment on post'
						);
					return data;
				} catch (error) {
					console.error(error);
